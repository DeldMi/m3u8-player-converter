...
Alguns servidores M3U8 exigem cookies, Referer, User-Agent ou autenticação. Esta versão não implementa esses cabeçalhos.
Alguns streams são protegidos por DRM; nesses casos o FFmpeg não deve ser usado para contornar a proteção.
Para publicar esta aplicação na internet, adicione autenticação e uma política de URLs permitidas. Aceitar URLs arbitrárias em um servidor público pode criar risco de SSRF.
O player no navegador pode funcionar mesmo quando o FFmpeg não consegue acessar o mesmo stream, por diferenças de CORS, headers ou autenticação.
